...
class LCIIS(object):
    ...
    def NewFock(self, fockTup, densTup):
        ...
        self.__fList.append(fockTup)
        shape = (self.__toOr.shape[0], -1)
        trFList = [self.__toOr.T.dot(fock.reshape(shape)) for fock in fockTup]
        trDList = [dens.reshape(shape).dot(self.__orTo) for dens in densTup]
